...
| `alert_essentials_saved` | Alert settings saved (Essentials path) |
| `family_share_created` | Family / share live link created |
...
4. `alert_essentials_saved` or `onboarding_step_complete` where `step = alerts`
5. `first_alert_test` or `onboarding_step_complete` where `step = alert_test`
6. `family_share_created` (optional retention / viral step)
...
2. `first_ingest`
3. `sensors_renamed`
4. `alert_essentials_saved`
5. `first_alert_test`
6. `family_share_created`
...
- `alert_essentials_saved` — freeze+email configured
- `first_alert_test` — trust / retention signal
- `family_share_created` — household sharing activation
