...
#else
#error "door_contact_ingest targets ESP32 / ESP8266."
#endif
...
bool postDoor(bool open) {
  if (WiFi.status() != WL_CONNECTED) return false;
  HTTPClient http;
  http.begin(INGEST_URL);
  http.addHeader("Content-Type", "application/json");
  char body[64];
  snprintf(body, sizeof(body), "{\"door1\":%s}", open ? "true" : "false");
...
void loop() {
  /* INPUT_PULLUP: LOW = contact closed = door shut */
  bool open = digitalRead(CONTACT_PIN) == HIGH;
  unsigned long now = millis();
  bool due = (now - lastPost >= INTERVAL_MS);
  bool changed = (int)open != lastSent;
  if (!due && !changed) return;
  lastPost = now;
  if (postDoor(open)) lastSent = (int)open;
}
